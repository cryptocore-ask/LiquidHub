#!/usr/bin/env node
/**
 * GenerateSafeBatch.js — Pool Standard
 *
...
const batch = {
    ...
    createdAt: Date.now(),
    meta: {
        name: 'Post-Deploy Configuration (Standard Pool)',
...
const poolName = process.env.RESEAU_BC_SHORT
    ? `${process.env.PROTOCOLE_SHORT}-${process.env.RESEAU_BC_SHORT}-${process.env.PAIR_TOKEN0}-${process.env.PAIR_TOKEN1}`
    : 'pool';
...
console.log('\n========================================');
console.log(' SAFE BATCH GENERATED');
console.log('========================================');
console.log(`\nFichier : ${outputFile}`);
