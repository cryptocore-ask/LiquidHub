...
class Rebalancer {
  ...
  async compoundPosition() {
    ...
    // Keep the same modest economic threshold as the main bot, without a TVL gate.
  ...
  async _runProgressiveRebalance(expectedDecisionHash = null) {
    ...
    if (status !== 0 && status !== 2) throw new Error(`unexpected progressive rebalance state: ${status}`);
    ...
    while (!completedElsewhere) {
      ...
      if (!swapNeeded || planAmount === 0n) {
        ...
        if (receipt) {
          ...
          throw new Error('progressive finalization returned no receipt and no final state');
        }
        break;
      }
  ...
  _shouldRefreshForPlanError(error) {
    ...
    return ['stale', 'cache', 'oracle', 'twap', 'price', 'minout', 'too little received', 'partialfill', 'partial fill', 'invalid chunk', 'cycle budget', 'reverse budget', 'use atomic', 'swapchunkabovecap', 'e38', 'e90', 'e91', 'e96', 'e93', 'e94']
